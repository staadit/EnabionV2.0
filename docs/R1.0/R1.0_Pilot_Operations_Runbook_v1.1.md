# R1.0 Pilot Operations Runbook (v1.0) â€” Pilot-Mode Production on One VPS
...
3) Run smoke checks (must pass):
   - `https://api.enabion.com/health` â†’ 200
   - `https://enabion.com/api/health` â†’ 200
   - Create Intent (L1) â†’ ensure `INTENT_CREATED` exists in timeline
   - Attempt L2 access before NDA â†’ must be blocked
   - Accept Mutual NDA â†’ L2 visible; `NDA_ACCEPTED` event exists
...
3) Confirm edge proxy still serves:
   - maintenance page (if configured) OR a clear â€œpilot offlineâ€ response
...
Rollback is always â€œdeploy an older tagâ€:
