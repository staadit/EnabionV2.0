...
Naming note: the **prod environment is the pilot stack** (compose project `enabion_pilot`) because prod runs only during pilot windows.

## VPS architecture overview

- Edge/Traefik is always-on and routes by host.
- Dev stack is always-on (`infra/docker-compose.prod.yml`).
- Pilot stack is on-demand (`infra/docker-compose.prod.pilot.yml`, project `enabion_pilot`).
- Isolation is via compose project name + separate env folders + separate volumes.
...
Internet
  ...
  v
[Traefik / edge]
  |-- dev.enabion.com / api.dev.enabion.com -> DEV stack
  |-- enabion.com / api.enabion.com         -> PILOT stack (on demand)
```

Key VPS paths:
- Repo: `/srv/enabion/prod/repo`
- Dev env: `/srv/enabion/dev/env/{db,backend,frontend}.env`
- Pilot env: `/srv/enabion/prod/env/{db,backend,frontend}.env`
- Dev volumes: `/srv/enabion/_volumes/prod/{postgres,blobstore}`
- Pilot volumes: `/srv/enabion/_volumes/pilot/{postgres,blobstore}`
- Pilot DB backups: `/srv/enabion/_backups/prod/postgres/`
 - Frontend BACKEND_URL (edge alias): dev `http://backend-dev:4000`, pilot `http://backend-pilot:4000`
...
1) **Cut an RC tag** from `main` (local):
   - `git checkout main`
   - `git pull --ff-only`
   - `git tag r1.0-rc.<n>`
   - `git push origin r1.0-rc.<n>`
   - Optional (after pilot go-live): `git tag r1.0-pilot.<n>` then `git push origin r1.0-pilot.<n>`
2) On VPS: deploy the tag and start pilot:
...
4) Confirm backups are possible (prod DB running):
   - take an on-demand DB dump from the VPS
...
1) Kill switch (immediate): stop pilot on VPS:
...
2) On VPS: checkout and restart the pilot stack:
   - `git checkout r1.0-rc.<k>`
   - `COMPOSE_PROJECT_NAME=enabion_pilot docker compose -f infra/docker-compose.prod.pilot.yml down`
   - `COMPOSE_PROJECT_NAME=enabion_pilot docker compose -f infra/docker-compose.prod.pilot.yml up -d --build`
...
- Pilot deploys are tag-only from `main` (`r1.0-rc.<n>`); do not deploy branches to prod.
...
- `INF-04` / `R1.0-PROD-001/002/003` cover the infrastructure and manual ops.
