...
### R1.0 scope (Model 1, NDA 0/1 only)
- Data model: **Model 1 - Standard** (L1 + selected L2) in Enabion multi-tenant cloud.
- NDA layers: **Layer 0 (No-NDA, L1)** and **Layer 1 (Mutual NDA for L2)** supported in R1.0 pre-sales.
- L3 appears only as a UI placeholder; no L3 data is processed or stored in R1.0.
- Models 2-3 (Shielded/Sovereign) and NDA Layers 2-3 are **future (R2+/R4+)**, not part of MVP.

This baseline aligns with the core principles in the Enabion Playbook:

- **Your Data, Your Control - Our Trust Layer** - Enabion cares primarily about structured context, events and trust signals, not full raw document archives. (Playbook/Implementation Plan reference)  
- **Model 1 - Standard as default** for SMEs that already use modern SaaS and do not need dedicated Shielded/Sovereign setups yet. (Playbook/Implementation Plan reference)  
...
- companies that need **fast value in pre-sales & collaboration**, not yet full regulated-sector controls. (Playbook/Implementation Plan reference)  
...
- organisations, users and roles (Owner / Manager / Contributor / Viewer), (Playbook/Implementation Plan reference)  
- Intents, pre-sales pipeline, basic matching, Avatars for Clarify & Match,  
- L1/L2 confidentiality levels and NDA Layers 0/1 (No-NDA Zone + Enabion Mutual NDA), (Playbook/Implementation Plan reference)  
- basic behavioural TrustScore MVP (profile completeness, responsiveness, intent completion). (Playbook/Implementation Plan reference)  

**Explicitly out of scope for R1.0 / Model 1:**

- Model 2 - Shielded and Model 3 - Sovereign (only architecture-ready in MVP, not delivered), (Playbook/Implementation Plan reference)  
- advanced AI Governance matri[x] per project type (comes with R4.0), (Playbook/Implementation Plan reference)  
- ODR v1 (formal Online Dispute Resolution) - appears with R3.0, not MVP, (Playbook/Implementation Plan reference)  
- full Hubs productisation (R3+),  
- financial escrow & advanced economic governance (R3+). (Playbook/Implementation Plan reference)  
...
As defined in the Playbook: (Playbook/Implementation Plan reference)  
...
- **Layer 1 - Enabion Mutual NDA**  
  Ecosystem-wide pre-sales NDA signed once by the organisation at onboarding, covering pre-sales interactions between parties that also accepted Mutual NDA. (Playbook/Implementation Plan reference)  

Layers 2-3 (Custom NDA FastTrack & External NDA via DocuSign/Adobe) are defined for future releases, not implemented in MVP. (Playbook/Implementation Plan reference)  
...
2. **Intents & pre-sales data**  
   ...
   - simple matching metadata (sectors, technologies, geography, language, size). (Playbook/Implementation Plan reference)  

3. **Event log (MVP subset)**  
   For MVP we log only events strictly required to support Clarify-Match-Commit in pre-sales: (Playbook/Implementation Plan reference)  
...
Consistent with the "Your Data, Your Control - Our Trust Layer" principle, R1.0 does **not require**: (Playbook/Implementation Plan reference)  
...
Model 1 uses the baseline architecture from the Implementation Plan: (Playbook/Implementation Plan reference)  
...
- **AI Gateway** - service that sends well-scoped tasks to LLMs (Intent Coach, avatar suggestions) and logs AI usage. (Playbook/Implementation Plan reference)  
...
- **Integrations** - primarily e-mail->Intent in R1.0; other integrations are future work. (Playbook/Implementation Plan reference)  
...
**Authorisation & roles** (Playbook/Implementation Plan reference)  
...
- Basic infrastructure monitoring (uptime, error rates, latency) with alerting on critical thresholds for MVP. (Playbook/Implementation Plan reference)  

### 6.5 AI & LLM usage controls

Aligned with AI Gateway & Avatars design & guardrails: (Playbook/Implementation Plan reference)  
...
- Avatars must be able to respond with **"I don't know / I need more data"** and should not fabricate unknown facts about organisations or people (behavioural requirement enforced via prompt design & testing). (Playbook/Implementation Plan reference)  

### 6.6 Email->Intent - abuse & phishing

For the email->Intent integration (R1.0 scope): (Playbook/Implementation Plan reference)  
...
> Intent -> Avatars -> Matching -> NDA -> pre-sales pipeline. (Playbook/Implementation Plan reference)  
...
- Organisations can request anonymisation of closed Intents and associated events, except where legal retention obligations apply (e.g. billing logs). (Playbook/Implementation Plan reference)  
...
Detailed legal policy will be captured in the separate **Data Ownership & Deletion Policy** document mentioned in the Implementation Plan. (Playbook/Implementation Plan reference)  
...
- For non-EU customers, we will initially apply the same baseline and extend region-specific controls with R4.0 (multi-jurisdiction support). (Playbook/Implementation Plan reference)  
...
1. **No dedicated Model 2-3 enforcement yet**  
   - we do not support fully shielded or sovereign deployments; those require additional work and are not part of MVP. (Playbook/Implementation Plan reference)  
...
4. **No full ODR / dispute process**  
   - since Deliver/Execution is not part of R1.0, we do not yet provide formal dispute workflows or evidence packages (these come with R2.0-R3.0). (Playbook/Implementation Plan reference)  
...
This document is a **baseline** for R1.0. The path forward is: (Playbook/Implementation Plan reference)  
...
#### 6.4.1 Logging scope (R1.0, pre-sales only)
- Logged events: INTENT_CREATED, INTENT_UPDATED, AVATAR_SUGGESTION_ISSUED, AVATAR_SUGGESTION_ACCEPTED, AVATAR_SUGGESTION_REJECTED, MATCH_LIST_CREATED, NDA_ACCEPTED (Layer 1), COMMIT_DECISION_TAKEN.
- Not logged in R1.0: ISSUE_*, DISPUTE_*, delivery/ODR events (arrive with Deliver/Trust Rooms in R2.0+).
