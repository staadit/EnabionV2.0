...
- R1.0 focuses primarily on:
  ...
  - plus very light Deliver/Expand as _pipeline statuses only_ (no project containers yet).

Pipeline column names in R1.0 mirror the early lifecycle:

- **New -> Clarify -> Match -> Commit -> Won -> Lost**.

`Deliver` and `Expand` exist only as concepts and future phases; they are _not_ implemented as full flows in R1.0.
...
- Number of organizations (X and Y)

- Number of active users

- Number of Intents created per org and per month
...
- Avatar usage:
  ...
  - Number of AI suggestions per Intent

  - Percentage of suggestions accepted, rejected.
...
### Security & Privacy baseline (Model 1 - R1.0) - canonical references

- Technical baseline: docs/Security_Privacy_Model1_MVP.md (roles/RBAC, logging scope, encryption, NDA 0/1).
- Customer-facing note: docs/Security_Privacy_Note_Model1_R1.0.md.
- Logging scope (R1.0): log only pre-sales events (INTENT_*, AVATAR_SUGGESTION_*, MATCH_LIST_CREATED, NDA_ACCEPTED, COMMIT_DECISION_TAKEN); no ISSUE_*/DISPUTE_* until R2.0.
- Scope guardrail: only Model 1 + NDA Layers 0/1; L3 is placeholder only; Models 2-3 and NDA Layers 2-3 are future (R2+/R4+).

### NDA references (R1.0)

- Legal text (Mutual NDA Layer 1): docs/legal/enabion_mutual_nda_layer1_legal_R1.0.md.
- Product/UX copy (L0/L1): docs/legal/enabion_mutual_nda_layer1_L0-L1_product_copy_R1.0.md.`n- Align NDA fields/events with docs/BCOS_Data_Event_Model_v1.md (NDA_ACCEPTED, confidentiality levels).

### Out of scope - R1.0 (shifted to later releases)

- Deliver/Expand beyond pre-sales status (Won/Lost) -> R2.0 (BCOS Containers & Trust Rooms).
- ISSUE_* / DISPUTE_* events and ODR flows -> R2.0/R3.0.
- Model 2 - Shielded / Model 3 - Sovereign (only hooks in arch) -> R2+/R4+.
- EnableMark (TM), TrustGraph runtime, Hubs v1 -> R3+.
