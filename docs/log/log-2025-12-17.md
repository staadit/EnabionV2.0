...
Progress — 2025-12-18 19:15:00 UTC

R1.0-BLOBSTORE-SEC-001 #87: enforced MIME allowlist + max upload defaults, added signed URL download path for S3 (TTL 5m) with streaming/decrypt for encrypted/local, and emit audit events for upload/download (#84) with NDA gate intact. Updated docs (.env example, docs/R1.0/R1_0_infra_blobstore.md, Security_Privacy_Note_Model1_R1.0.md). Tests: `npm test` (blobstore, attachments, contracts, tenant, NDA leak, API) green.

Progress — 2025-12-18 20:10:00 UTC

Frontend: added /api/attachments/[id] proxy that handles new backend response shape ({ signedUrl, expiresAt, contentType }) by redirecting to signed URLs (with expiry header) or streaming fallback; .env example added (BACKEND_URL).

Progress — 2025-12-18 20:30:00 UTC

Set BACKEND_URL for frontend: docker-compose.dev uses BACKEND_URL=http://backend:4000, NEXT_PUBLIC_BACKEND_URL=http://localhost:4000; added env/frontend.env template (BACKEND_URL internal, NEXT_PUBLIC_BACKEND_URL public API host) for staging/prod compose.

Progress — 2025-12-18 21:00:00 UTC

R1.0-HEALTH-001 #98: added healthchecks for dev/stage compose (db pg_isready; backend /health; frontend /api/health with depends_on healthy). Added README note with expected endpoints. #87 already closed; #98 ready to move after verification.

Progress - 2025-12-18 21:20:00 UTC

Staging routing fix: hooked backend/frontend to Traefik edge with dev.enabion.com / api.dev.enabion.com host rules (infra/docker-compose.staging.yml labels + edge network). Redeploy on VPS with updated compose to clear 404 on dev domain.

Progress - 2025-12-18 21:45:00 UTC

Staging verified 200 OK: env files placed under /srv/enabion/env, compose rebuilt (backend/frontend/db) on edge network with Traefik host rules; curl https://dev.enabion.com and https://api.dev.enabion.com/health now return 200. Traefik connected to enabion_edge.
